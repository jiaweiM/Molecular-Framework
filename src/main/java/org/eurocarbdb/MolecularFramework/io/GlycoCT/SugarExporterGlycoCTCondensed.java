...
//		 concatenate main CT - substrings		
    ...
    private String exportNonMS() throws GlycoVisitorException {
        ...
        String t_strResult = "";
        for (NonMonosaccharide m_aNonM : this.m_aNonMS) {
            t_iCounter++;
            NonMonosaccharide t_objNon = m_aNonM;
            t_strResult += "NON" + t_iCounter.toString() + "\n";
            Linkage t_objLink = null;
            if (t_objNon.getParentEdge() == null) {
                if (t_objNon.getChildEdges().size() != 1) {
                    throw new GlycoVisitorException("NonMonosaccharide units with more or less than one connection " +
                            "are not allowed for GlycoCT{condensed}");
                }
                for (GlycoEdge t_objEdge : t_objNon.getChildEdges()) {
                    if (t_objEdge.getGlycosidicLinkages().size() != 1) {
                        throw new GlycoVisitorException("NonMonosaccharide units with more or less than one linkage " +
                                "are not allowed for GlycoCT{condensed}");
                    }
                    for (Linkage linkage : t_objEdge.getGlycosidicLinkages()) {
                        t_objLink = linkage;
                    }
                ...
                if (t_objNon.getChildEdges().size() > 0) {
                    throw new GlycoVisitorException("NonMonosaccharide units with parent and child connection are not" +
                            " allowed for GlycoCT{condensed}");
                }
                GlycoEdge t_objEdge = t_objNon.getParentEdge();
                if (t_objEdge.getGlycosidicLinkages().size() != 1) {
                    throw new GlycoVisitorException("NonMonosaccharide units with more or less than one linkage are " +
                            "not allowed for GlycoCT{condensed}");
                }
                for (Linkage linkage : t_objEdge.getGlycosidicLinkages()) {
                    t_objLink = linkage;
                }
