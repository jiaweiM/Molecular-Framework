...
public class GlycoVisitorNamespaceCarbbank implements GlycoVisitor {
    ...
    public void visit(Monosaccharide aObjMonosaccharid) throws GlycoVisitorException {
        throw new GlycoVisitorException("Monosaccharide is not supported for exporting: Use Namespace translation " +
                "first.");
    }

    public void visit(NonMonosaccharide aObjResidue) throws GlycoVisitorException {
        throw new GlycoVisitorException("NonMonosaccharide is not supported for exporting: Use Namespace translation " +
				"first.");
    }
    ...
    public void visit(SugarUnitAlternative aObjAlternative) throws GlycoVisitorException {
        throw new GlycoVisitorException("Alternative residues are not supported for exporting: Use Namespace " +
				"translation first.");
    }
    ...
    public void visit(UnvalidatedGlycoNode a_node) throws GlycoVisitorException {
        ...
        try {
            if ((t_name.charAt(0) == 'D' || t_name.charAt(0) == 'L') && (t_name.charAt(t_name.length() - 1) == 'a' ||
					t_name.charAt(t_name.length() - 1) == 'b')) {
                // it is a monosaccharide
                String t_nameNew = t_name.charAt(t_name.length() - 1) + "-" + t_name.charAt(0) + "-" + t_name
						.substring(1, t_name.length() - 1);
